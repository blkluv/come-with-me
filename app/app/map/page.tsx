...
export default function MapPage() {
  ...
  return (
    <div className="relative h-screen overflow-hidden">
      {/* Map */}
      <MapComponent onSelectPlace={setSelectedPlace} />
      ...
      >
        <div className="bg-dark/95 backdrop-blur border-t border-white/10 rounded-t-3xl">
          ...
          {/* Places list */}
          <div className="flex flex-col gap-0 pb-28 max-h-72 overflow-y-auto">
            {PLACES.map((place, i) => (
              <button
                key={place.id}
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl bg-gradient-to-br ${place.gradient} flex-shrink-0 flex items-center justify-center`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/40 text-xs font-jakarta">
                    {place.neighborhood} · {place.creator}
                  </p>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className="text-lime text-xs font-jakarta font-semibold">★ {place.rating}</span>
                  <span className="text-white/20 text-xs font-jakarta">0.{Math.floor(Math.random() * 9 + 1)} mi</span>
                </div>
              </button>
            ))}
          </div>
